...
impl<'a> Stream for AsyncStreamDataIterator<'a> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        // Use the common encrypt function.
        match common::encrypt_chunk(&mut session, chunk) {
    ...
    R: AsyncRead + Unpin,
{
    ...
    pub async fn copy_to<W: AsyncWrite + Unpin>(&mut self, writer: &mut W) -> Result<u64> {
        ...
        loop {
            ...
            // Encrypt the chunk using the common helper.
            let encrypted = {
                let mut session = self.session.lock().unwrap();
                common::encrypt_chunk(&mut session, &buffer[..bytes_read])?
            };
    ...
    W: AsyncWrite + Unpin,
{
    ...
    pub async fn process_chunk(&mut self, chunk: &[u8]) -> Result<Vec<u8>> {
        let decrypted = {
            ...
            common::decrypt_chunk(&mut session, chunk)?
        };
    ...
    pub async fn process_chunks<S, F>(&mut self, mut chunks: S) -> Result<usize>
    where
...
mod tests {
    ...
    async fn test_stream_reader() -> Result<()> {
        ...
        let mut reader_stream = sender.stream_reader().await;
    ...
    async fn test_copy_to() -> Result<()> {
        ...
        let _sent = sender.copy_to(&mut client_write).await?;
        let _received = receiver.process_reader(&mut server_read).await?;
        assert_eq!(output.len(), data.len());
        assert_eq!(output, data);
        let reassembled = receiver.reassembled_data().unwrap();
        assert_eq!(reassembled.len(), data.len());
    ...
    async fn test_extension_traits() -> Result<()> {
        ...
        let _sent = sender.copy_to(&mut tokio::io::Cursor::new(Vec::new())).await?;
        Ok(())
    }
}
